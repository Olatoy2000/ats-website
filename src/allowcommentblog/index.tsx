...
export default function AllowCommentBlog() {
  const BLOGS = [
    {
      id: 1,
        ...
        {BLOGS.map((item, idx) => (
          <section className="mb-12" key={idx}>
            ...
            <section className="grid gap-16 grid-cols-[1fr_300px]">
              <article className="leading-8 text-[1.125rem] text-justify text-light-completeBlack font-normal">
                {item.blog_text}
              </article>
              <aside>
                  ...
                  <article className="flex gap-4 flex-col">
                    {item.comments.map((el, id) => (
                      <Fragment key={id}>
                        ...
                        <div className="break-words">{el.comment}</div>
                      </Fragment>
                    ))}
                  </article>
